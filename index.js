...
class Seasons {
   ...
   */
  next() {
    const seasonResult = this.seasons[this.currentSeason]
...
class Car {
   ...
   */
  drive(distance) {
    const carGo = this.tank * this.mpg
    if(distance <= carGo) {
      this.odometer = this.odometer + distance
      this.tank = this.tank - (distance / this.mpg)
    } else { 
    this.odometer = this.odometer + carGo
    this.tank = 0
    }
   ...
   */
  refuel(gallons) {
    const FillErUp = this.tankSize - this.tank 
    if (gallons <= FillErUp) {
